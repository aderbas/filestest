...
COMMENT(Protocolfile,'Faz um IF que vai ser avaliado como false, mas o teste nao sera considerado fail');
IF(CHECK(Testdata,'Data/Medidor=True');)
    COMMENT(Protocolfile,'O status do teste continua 0 (pass)');
END;

COMMENT(Protocolfile,'Faz um teste que vai falhar');
CHECK(Testdata,'Data/Medidor=True');
